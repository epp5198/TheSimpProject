...
declare variable $allCatches := $simpsonsAll//music => distinct-values();
declare variable $distCatches := $simpsonsAll//music ! normalize-space() ! lower-case(.) ! replace(., "'", '') => distinct-values();
...
<svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
    <g transform="translate(50, 20)">
    {
        ...
        let $countThisCatch := sum(for $c in $matchingCatch return count($simpsonsAll//Q{}music[. = $c]))
        return
        <g id="{$d}">
